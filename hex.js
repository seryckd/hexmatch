...
var HEX = (function() {
  ...
  xycoords = function(q, r) {
    ...
    var y = Math.floor(hexSize * 3/2 * r) 

    return { x:x+xyoffset.x, y:y+xyoffset.y};
  };
  ...
  return {
	...
	init: function(dx, dy, numcol_, numrow_, width, height) {
		...
		for(var r=0; r<numrow; ++r) {
			var numcols = (r%2) ? numcol-1 : numcol;
			for(var col=0; col<numcols; ++col) {        
				var q = -Math.floor(r/2) + col;
				var hex = new HexData(q, r);
    ...
    removeRandom: function(num) {
      ...
      while(num > 0) {
        ...
        var r = Math.round(Math.random() * (numrow-1));
      
        if (hexes[hash(q,r)] != null) {
    ...
    neighbours: function(hex, apply, direct) {
      ...
      if (direct == null) {
        ...
        // using modulo 6
        direct = prevDirection(direct); total=3;
      }
      
      while(count++ < total) {
	    var newq = hex.axial.q + directions[direct].q;
	    var newr = hex.axial.r + directions[direct].r;
					  
	    var next = getHex(newq, newr);             
        if (next != null)
          apply(next, direct);
     ...
     */
		...
		var t1 = y / hexSize;
		var t2 = Math.floor(x1 + t1);
		var r = Math.floor((Math.floor(t1 - x1) + t2) / 3); 
		var q = Math.floor((Math.floor(2 * x1 + 1) + t2) / 3) - r;
