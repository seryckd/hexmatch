...
HEX.removeRandom(2);
HEX.each(function(hex) { hex.setDisplayCoords(hex.xycoords); } );

IMAGE.load("marker", "images/marker.gif");

SPRITE.load("hexes", "images/hextiles.png", [
  { name: "clay", height:64, width:55, x:0, y:0 },
  { name: "water", height:64, width:55, x:55, y:0 },
  { name: "wheat", height:64, width:55, x:110, y:0 },
  { name: "ore", height:64, width:55, x:165, y:0 },
  { name: "desert", height:64, width:55, x:220, y:0 },
  { name: "sheep", height:64, width:55, x:275, y:0 },
  { name: "wood", height:64, width:55, x:330, y:0 },
  { name: "dead", height:64, width:55, x:385, y:0 }
]);

SPRITE.load("twinkle", "images/twinkle.png", [
  { name: "1", height:40, width:40, x:0, y:0 },
  { name: "2", height:40, width:40, x:40, y:0 },
  { name: "3", height:40, width:40, x:80, y:0 },
  { name: "4", height:40, width:40, x:120, y:0 },
  { name: "5", height:40, width:40, x:160, y:0 },
  { name: "6", height:40, width:40, x:200, y:0 },

  { name: "7", height:40, width:40, x:0, y:40 },
  { name: "8", height:40, width:40, x:40, y:40 },
  { name: "9", height:40, width:40, x:80, y:40 },
  { name: "10", height:40, width:40, x:120, y:40 },
  { name: "11", height:40, width:40, x:160, y:40 },
  { name: "12", height:40, width:40, x:200, y:40 },

  { name: "13", height:40, width:40, x:0, y:80 },
  { name: "14", height:40, width:40, x:40, y:80 },
  { name: "15", height:40, width:40, x:80, y:80 },
  { name: "16", height:40, width:40, x:120, y:80 },
  { name: "17", height:40, width:40, x:160, y:80 },
  { name: "18", height:40, width:40, x:200, y:80 },

  { name: "19", height:40, width:40, x:0, y:120 },
  { name: "20", height:40, width:40, x:40, y:120 }
],
[
  { name: "shine1", durationms: 100, spritenames: [
    "15", "14", "13",
    "12", "11", "10", "9", "8", "7",
    "7", "8", "9", "10", "11", "12",
    "13", "14", "15"
...
function update(interval) {
	...
	if (possible.hex != null && isUpdate) {
		...
		possible.matches.forEach(function(hex) {
			...
			do {
				var direction = hex.blockeddirection;

				do {
					direction = Math.round(Math.random() * 7);
				} while(hex.blockeddirection == direction);

				var of = directions[direction];

				var dcoords = hex.dcoords;
				x = dcoords.x + of.x;
				y = dcoords.y + of.y;
	...
	particles.update(interval,  {width:canvas.width, height:canvas.height});
}
...
var render = function() {
	...
	if (possible.hex != null) {
		...
		if (match) {
			...
			var ref = shineanim.sprite();
			var xoffset = (HEX.hexWidth() - ref.width) / 3;
			var yoffset = (HEX.hexHeight() - ref.height) / 3;
			ctx.drawImage(ref.image,
				ref.x,ref.y,ref.width,ref.height,
...
//	});
	...
	var spos = canvas.width - 1.5 * digitWidth;
	for(var count = sscore.length-1; count >= 0; --count) {
